...
public class SampleApplicationService {
    ...
    public void runException() {
        log.info("start");
        applicationEventPublisher.publishEvent(new CreatedOrderEvent(Integer.toString(atomicInteger.getAndIncrement())));
        log.info("end");
    ...
    @Transactional
    public void run() {
        applicationEventPublisher.publishEvent(new CreatedOrderEvent(UUID.randomUUID().toString()));
        applicationEventPublisher.publishEvent(new CreatedOrderEvent(UUID.randomUUID().toString()));
        applicationEventPublisher.publishEvent(new CreatedOrderEvent(UUID.randomUUID().toString()));
    }
