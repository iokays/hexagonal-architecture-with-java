 ...
 * 客户的邮箱地址
 *
 * @param email {@link Integer}邮箱地址的值
 */
public record EmailAddress(String email) implements ValueObject<EmailAddress> {
     ...
     */
    public EmailAddress {
        Validate.isTrue(StringUtils.isNotEmpty(email), "邮箱地址不能为空");
        Validate.isTrue(isValidEmailAddress(email), "邮箱地址不合法");
    }
